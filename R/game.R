...
evaluate_game <- function(game_id, schedule, injuries){
  ...
  status_dates <- c(sort(unique(game_injuries$date)), lubridate::as_date(game$game_time))
  status_days <- lubridate::wday(as.character(status_dates), label = TRUE, week_start = 1, abbr = FALSE) |>
    as.character() |>
